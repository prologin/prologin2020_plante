...
const std::vector<position> player_plant_depot = {{11, 10}, {12, 11}};
...
TEST_F(ApiTest, test_action_baffer_after_move)
{
    for (int player : {0, 1})
    {
        EXPECT_EQ(OK, p_api[player]->baffer(player_plant[player],
    ...
    for (int player : {0, 1})
    {
        ...
        EXPECT_EQ(OK, p_api[player]->depoter(player_plant[player],
                                             player_plant_depot[player]));

        EXPECT_EQ(DEJA_BAFFEE,
                  p_api[player]->baffer(player_plant_depot[player],
                                        player_plant_depot[player]));
