...
plants = [[ False for _ in range(MAP_SIZE)]  for _ in range(MAP_SIZE)]
...
for row in range(MAP_SIZE):
    ...
    if len(cells) != MAP_SIZE:
        exit(f'La grille n\'est pas de la bonne largeur: {len(cells)}')

    for col, cell in enumerate(cells):
        res = cell.split(',')
        if len(res) != RESSOURCE_COUNT:
            exit(f'La case {cell} n\'a pas le bon nombre de ressources:',len(res))
...
for player in range(PLAYER_COUNT):
    ...
    for _ in range(nb_plants):
        ...
        if len(stats) != 7 + RESSOURCE_COUNT:
            exit(f'La plante {stats} n\'a pas le bon nombre de caractèristiques',len(stats))
